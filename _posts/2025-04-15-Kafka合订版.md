...
Kafka 的可靠性（Reliability）是端到端的，需要从 **Producer（生产者）**、**Broker（服务端）** 和 **Consumer（消费者）** 三个层面协同配置才能保证。

### 1. 生产者端 (Producer)
- **acks=all (或 -1)**: 这是最严格的确认机制。Leader 接收到消息后，必须等待所有 ISR（In-Sync Replicas，同步副本）都成功写入日志，才向 Producer 返回成功。这保证了只要 ISR 中还有一个副本存活，数据就不会丢失。
- **retries > 0**: 网络抖动是常态，开启重试可以自动处理瞬时的发送失败。
- **enable.idempotence=true (幂等性)**: 开启后，Producer 会被分配一个 PID，且每条消息带序列号。Broker 会自动去重，保证即使重试，消息在 Partition 内也只会被持久化一次，实现了 **Exactly-Once** (单分区) 语义。

### 2. 服务端 (Broker)
- **replication.factor >= 3**: 设置副本因子为 3，即一份数据存 3 份，允许挂掉 2 个 Broker 仍不丢数据。
- **min.insync.replicas > 1**: 仅仅设置 `acks=all` 是不够的。如果 ISR 列表里只剩下 Leader 自己，`acks=all` 也就退化成了 `acks=1`。必须强制要求至少有 2 个副本确认写入，才能视为成功。通常设置 `min.insync.replicas = 2`。
- **unclean.leader.election.enable=false**: 禁止非 ISR 节点的“不洁选举”。虽然这可能会在极端情况下导致整个分区不可用，但它防止了数据丢失（一致性 > 可用性）。

### 3. 消费者端 (Consumer)
- **关闭自动提交 (enable.auto.commit=false)**: 默认的自动提交是每隔一段时间（如 5秒）提交一次。如果在提交前机器挂了，会重复消费；如果在处理完之前就提交了（异步处理场景），则会**丢失消息**。
- **手动提交 Offset**: 业务逻辑必须遵守 **“先处理数据，后提交 Offset”** 的原则，确保消息确实被消费成功了。

### 总结
要想做到不丢数据，标准的生产环境配置组合通常是：
> `acks=all` + `min.insync.replicas=2` + `replication.factor=3` + `手动提交 Offset`
